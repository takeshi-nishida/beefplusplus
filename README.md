...
+ 2026/02/04
    + （教員向け）期間設定で 9999/12/31 をクリックすると「今日にする」ボタンがポップアップ
    + （教員向け）お知らせ開封確認・資料閲覧状況確認に「見た人の割合」と「日時の可視化」を追加
    + （教員向け）「コース参加者登録」画面に簡易的なグループ分け機能を追加
    + （教員向け）「テストフィードバック 登録・編集」に「採点ブースト機能」を追加